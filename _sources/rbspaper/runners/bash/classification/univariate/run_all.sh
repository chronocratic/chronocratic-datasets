...
#
# Usage:
#   RBS_PROJECT_ROOT=/path/to/repo ./runners/bash/classification/univariate/run_all.sh [options]
#
...
# Query registry for UCR dataset list
DATASETS=()
while IFS= read -r line; do
  DATASETS+=("$line")
done < <(cd "$PROJECT_ROOT" && uv run python -c "
from src.rbspaper.data.data_setup import get_datasets_names
for name in get_datasets_names('ucr', form='list'):
    print(name)
")

if [ "${#DATASETS[@]}" -eq 0 ]; then
  echo "[ERROR] No classification/univariate datasets found in registry" >&2
  exit 1
fi
...
for exp_id in "${EXPERIMENTS[@]}"; do
  ...
  for dataset_name in "${DATASETS[@]}"; do
    ...
    if bash "$RUNNERS_BASE/classification/univariate/run_on.sh" "$exp_id" "$dataset_name" "$@"; then
