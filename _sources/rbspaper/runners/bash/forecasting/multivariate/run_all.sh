#!/usr/bin/env bash
#
# run_all.sh — Run all experiments on forecasting/multivariate datasets via HPC.
#
# Queries the Python registry at runtime for both experiment and dataset lists.
#
# Usage:
#   RBS_PROJECT_ROOT=/path/to/repo ./runners/bash/forecasting/multivariate/run_all.sh [options]
#
...
for exp_id in "${EXPERIMENTS[@]}"; do
  ...
  for dataset_name in "${DATASETS[@]}"; do
    ...
    if bash "$RUNNERS_BASE/forecasting/multivariate/run_on.sh" "$exp_id" "$dataset_name" "$@"; then
