#!/usr/bin/env bash
#
# run_on.sh — Run an experiment on a single forecasting/multivariate dataset via HPC.
#
# Usage:
#   RBS_PROJECT_ROOT=/path/to/repo ./runners/bash/forecasting/multivariate/run_on.sh <experiment_id> <dataset_name> [options]
#
...
bash "$RUNNERS_BASE/hpc_submit_single.sh" "$experiment_id" "$dataset_name" --forecasting_mode multivariate "$@"
