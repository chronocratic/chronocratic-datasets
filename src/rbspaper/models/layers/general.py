"""General neural network layers (Banded Fourier Layer)."""
...
class BandedFourierLayer(nn.Module):
    """Banded Fourier Layer for applying banded Fourier transform to input tensors.

    Applies a learnable Fourier transform restricted to a specific frequency band.

    Args:
        in_channels: Number of input channels.
        out_channels: Number of output channels.
        band: Index of the frequency band to process.
        num_bands: Total number of frequency bands.
        length: Length of the input sequence (default 201).
    """
    ...
    def forward(self, input_tensor: torch.Tensor) -> torch.Tensor:
        """Forward pass through the Banded Fourier Layer.

        Args:
            input_tensor: Input tensor of shape (batch_size, time_steps, in_channels).

        Returns:
            Output tensor after the banded Fourier transform.
        """
    ...
    def _apply_fourier_transform(self, input_tensor: torch.Tensor) -> torch.Tensor:
        """Apply the learnable Fourier transform within the assigned frequency band.
    ...
    def reset_parameters(self) -> None:
        """Initialize the layer parameters using Kaiming uniform for weights."""
