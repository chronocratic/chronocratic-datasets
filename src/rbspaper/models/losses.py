"""Loss functions for time series contrastive representation learning."""
...
def _compute_contrastive_loss_logits(
    ...
    Args:
        combined_instance: Concatenated instance embeddings of shape (T, 2B, C).
        dimension_size: Size of the dimension along which embeddings are concatenated.
        indexing_factor: Index tensor for computing the loss.
...
def instance_contrastive_loss(instance_1: torch.Tensor, instance_2: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        instance_1: First batch of sequences (batch_size, seq_len, features).
        instance_2: Second batch of sequences (batch_size, seq_len, features).

    Returns:
        Instance contrastive loss tensor.
    """
...
def temporal_contrastive_loss(instance_1: torch.Tensor, instance_2: torch.Tensor) -> torch.Tensor:
    """Compute temporal-level contrastive loss.
    ...
    Returns:
        Temporal contrastive loss tensor.
    """
...
def hierarchical_contrastive_loss(
    ...
    Args:
        instance_1: First batch of sequences (batch_size, seq_len, features).
        instance_2: Second batch of sequences (batch_size, seq_len, features).
        alpha: Weight of the instance contrastive loss (default 0.5).
        temporal_unit: Temporal unit level from which to apply temporal loss (default 0).

    Returns:
        Hierarchical contrastive loss tensor.
    """
    ...
    while instance_1.size(1) > 1:
        ...
        if hierarchy_level >= temporal_unit and 1 - alpha != 0:
...
def triplet_contrastive_loss(
    ...
    """Calculate triplet contrastive loss from anchor, positive, and negative representations.

    Args:
        anchor: Anchor representations of shape (batch_size, embedding_dim).
        positive: Positive representations of shape (batch_size, embedding_dim).
        negative: Negative representations of shape
            (batch_size, num_negatives, embedding_dim).
        negative_penalty: Penalty coefficient for the negative samples (default 1.0).
...
def pairwise_distance_triplet_contrastive_loss(
    ...
    """Calculate pairwise-distance triplet contrastive loss.

    Args:
        anchor: Anchor representations of shape (batch_size, embedding_dim).
        positive: Positive representations of shape (batch_size, embedding_dim).
        negative: Negative representations of shape (batch_size, embedding_dim).
        margin: Margin for the triplet loss (default 1.0).
        p: Norm degree for pairwise distance calculation (default 2.0).

    Returns:
        Mean pairwise distance triplet loss.
    """
...
def info_nce_loss(
    ...
    Args:
        z1: First set of embeddings (batch_size, seq_len, embedding_dim).
        z2: Second set of embeddings (batch_size, seq_len, embedding_dim).
        pooling: Pooling method - 'max' or 'mean' (default 'max').
        temperature: Temperature parameter for scaling the logits (default 1.0).
    ...
    if pooling == 'max':
        ...
        message = f'Unsupported pooling method: {pooling!r}'
...
def local_info_nce_loss(
    z1: torch.Tensor, _z2: torch.Tensor, pooling: str = 'max', temperature: float = 1.0, k: int = 16
) -> torch.Tensor:
    """Compute local InfoNCE loss for contrastive learning.

    Args:
        z1: First set of embeddings (batch_size, seq_len, embedding_dim).
        _z2: Second set of embeddings (unused; present for interface compatibility).
        pooling: Pooling method - 'max' or 'mean' (default 'max').
        temperature: Temperature parameter for scaling the logits (default 1.0).
        k: Number of local crops (default 16).
    ...
    start = int(rng.integers(low=0, high=sequence_length - crop_length + 1))
...
def sliding_local_info_nce_loss(
    ...
    Args:
        ...
        temperature: Temperature parameter for scaling the logits (default 1.0).
        k: Number of local crops (default 16).
        sliding: Sliding window step size (default 16).
        negative_number: Number of negative samples (default 16).

    Returns:
        Sliding local InfoNCE loss.

    Raises:
        ValueError: If pooling method is unsupported.
    """
    rng = np.random.default_rng()
    sequence_length = z1.size(1)  # Batch size x Sequence length x Embedding dimension
    ...
    start = int(rng.integers(low=0, high=crop_length + 1))
    while start < sequence_length - crop_length:
        anchors.append(z1[:, start : start + crop_length, :])
        pos_start = int(
            rng.integers(
                low=max(start - crop_length, 0),
                high=min(start + crop_length, sequence_length - crop_length) + 1,
            )
        )
        ...
        while len(neg) < negative_number:
            neg_start = int(rng.integers(low=0, high=sequence_length - crop_length + 1))
            if (
...
def l1_out_loss(
    z1: torch.Tensor, z2: torch.Tensor, pooling: str = 'max', _temperature: float = 1.0
) -> torch.Tensor:
    """Compute L1out loss for contrastive learning.

    Args:
        z1: First set of embeddings (batch_size, seq_len, embedding_dim).
        z2: Second set of embeddings (batch_size, seq_len, embedding_dim).
        pooling: Pooling method - 'max' or 'mean' (default 'max').
        _temperature: Temperature (unused; present for interface compatibility).
...
def similarity_loss(
    ...
    """Compute similarity loss between paired embeddings.
    ...
    Returns:
        Similarity loss (mean of positive pairs).
...
def _compute_gaussian_kernel(
    ...
    Args:
        source: Source embeddings.
        target: Target embeddings.
        kernel_mul: Kernel multiplier (default 2.0).
        kernel_num: Number of kernels (default 5).
        fix_sigma: Fixed sigma value for the Gaussian kernel (default None).
    ...
    Raises:
        ValueError: If bandwidth computation yields non-positive values.
    """
    ...
    if bandwidth <= 0:
        message = f'Error in function {__name__}: bandwidth is non-positive; got {bandwidth}'
        raise ValueError(message)
...
def maximum_mean_discrepancy_with_gaussian_kernel_loss(
    ...
    Args:
        source: Source embeddings (batch_size, seq_len, embedding_dim).
        target: Target embeddings (batch_size, seq_len, embedding_dim).
        kernel_mul: Kernel multiplier (default 2.0).
        kernel_num: Number of kernels (default 5).
        fix_sigma: Fixed sigma value for the Gaussian kernel (default None).
        pooling: Pooling method - 'max' or 'mean' (default 'max').
