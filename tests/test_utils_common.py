"""Tests for common utility functions (UTI-01, UTI-03).
...
def test_flatten_two_arrays() -> None:
    """UTI-01: flatten_list_of_np_arrays concatenates two arrays into 1-D."""
    result = flatten_list_of_np_arrays(
        list_of_np_arrays=[np.array([1, 2]), np.array([3, 4])]
    )
    assert list(result) == [1, 2, 3, 4]


def test_flatten_float_arrays() -> None:
    """UTI-01: flatten_list_of_np_arrays preserves float dtype."""
    a = np.array([1.0, 2.0])
    b = np.array([3.0, 4.0])
...
def test_flatten_single_array() -> None:
    """UTI-01: flatten_list_of_np_arrays with single element list."""
